...
This script evaluates the V6.0 GRU model's "zero-shot" performance
on new, unseen regions (SET 2: Non-Growing Season).
MODIFIED TO SAVE PREDICTIONS AS JSON
...
PREDICTION_FILE_NAME = "SET2_non_growing_season_predictions.json"
...
FORECAST_BASE_DATE = pd.to_datetime("2025-10-22")
...
def run_evaluation():
    print("--- Starting SET 2 Evaluation (Zero-Shot) ---")
    ...
    forecast_start_date = FORECAST_BASE_DATE + pd.Timedelta(days=1)
    forecast_end_date = FORECAST_BASE_DATE + pd.Timedelta(days=7)
    input_start_date = FORECAST_BASE_DATE - pd.Timedelta(days=SEQUENCE_LENGTH - 1)
    ...
    print(f"Found {len(unique_field_ids)} new fields to evaluate.")
    ...
    for field_id in unique_field_ids:
        try:
            ...
            region = region_parts[1].replace('-', ' ').title() if len(region_parts) > 1 else "Unknown"
            ...
            input_df = field_df[(field_df['date'] >= input_start_date) & (field_df['date'] <= FORECAST_BASE_DATE)]
            if len(input_df) != SEQUENCE_LENGTH: continue

            truth_df = field_df[(field_df['date'] >= forecast_start_date) & (field_df['date'] <= forecast_end_date)]
            if len(truth_df) != 7: continue

            input_data = input_df[ALL_VARIABLE_COLS].values.astype(np.float32)
            scaled_input_data = scaler.transform(input_data)
            predicted_vectors = forecast_7_days(model, scaler, scaled_input_data)
            predicted_ndvi = predicted_vectors[:, TARGET_COLUMN_INDEX]
            ground_truth_ndvi = truth_df[TARGET_COLUMN_NAME].values
            mae = mean_absolute_error(ground_truth_ndvi, predicted_ndvi)
